...
set_proxy() {
    ...
    if command -v gsettings &> /dev/null; then
        ...
        kwriteconfig5 --file kioslaverc --group 'Proxy Settings' --key httpProxy "http://127.0.0.1:9050" >/dev/null 2>&1 || true
        kwriteconfig5 --file kioslaverc --group 'Proxy Settings' --key httpsProxy "http://127.0.0.1:9050" >/dev/null 2>&1 || true
...
fetch_ip() {
 curl --socks5-hostname 127.0.0.1:9050 -s https://api64.ipify.org || true
}
...
EOF
  ...
  while true; do
    ...
    echo -e "${BLUE}[Proxy]: 127.0.0.1:9050 🛰️${NC}"
    sleep "$ROTATION_TIME"
  done
